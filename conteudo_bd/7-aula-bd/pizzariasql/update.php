...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        $stmt->execute([ $nome, $email, $cel, $pizza, $cadastro, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$contatos['id_contato']?>" method="post">
        ...
        <input type="text" name="id" placeholder="" value="<?=$contatos['id_contato']?>" id="id_contato" disabled>
        ...
        <select name="pizza" id_contato="pizza">
                <?php
                $stmt = $pdo->prepare('SELECT * FROM pizza_sabores');
                $stmt->execute(); // Executa a consulta SQL
                $resultPizzas = $stmt->fetchAll();
                foreach ($resultPizzas as $pizzas) { ?>
                    <option value="<?= $pizzas['nome'] ?>"><?= $pizzas['nome'] ?></option>
                <?php } ?>
            </select>
